...
# 回测数据
backtest_dates = pd.date_range('2026-03-01', '2026-04-28', freq='D')
backtest_afternoon = [75,82,78,85,88,92,86,95,89,84,79,87,91,83,81,86,93,88,82,87,92,90,85,80,89,94,91,86,84,90,88,85,81,89,93,87,83,79,96,89,84,82,87,91,85,83,88,92,90,86,84,89,94,91,87,85,90,88,93]
...
with tab1:
    ...
    fig1.add_trace(go.Scatter(
        x=backtest_dates, y=backtest_afternoon,
        mode='lines+markers',
        name='下午时段专用模型',
    ...
    avg_acc = sum(backtest_afternoon) / len(backtest_afternoon)
    ...
    with col2:
        st.metric("最高准确率", f"{max(backtest_afternoon)}%")
    with col3:
        st.metric("最低准确率", f"{min(backtest_afternoon)}%")
    with col4:
        high_days = sum(1 for x in backtest_afternoon if x >= 83)
        st.metric("达标天数", f"{high_days}/{len(backtest_afternoon)}")
